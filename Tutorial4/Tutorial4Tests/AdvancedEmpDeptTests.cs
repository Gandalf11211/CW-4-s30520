...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnMaxSalary()
    {
        ...
        decimal? maxSalary = emps.Select(emp => emp.Sal)
                                 .Max(); 
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps, emp => emp.Mgr, m => m.EmpNo, (emp, m) => new { Employee = emp.EName, Manager = m.EName });
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps.Select(emp => new { EName = emp.EName, Total = emp.Sal + (emp.Comm ?? 0) }); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Select(emp => new { EName = emp.EName, DName = depts.Single(dept => dept.DeptNo == emp.DeptNo).DName, Grade = grades.Single(g => g.Losal <= emp.Sal && g.Hisal >= emp.Sal).Grade }); 
