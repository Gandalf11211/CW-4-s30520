...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(emp => depts.Any(dept => dept.Loc == "CHICAGO" && dept.DeptNo == emp.DeptNo))
                               .ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.Select(emp => new { Ename = emp.EName, Grade = grades.Single(g => g.Losal <= emp.Sal && emp.Sal <= g.Hisal).Grade });
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo)
                                                 .Select(g => new { DeptNo = g.Key, AvgSal = g.Average(sal => sal.Sal) });
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(emp => emp.Sal > emps.Where(x => x.DeptNo == emp.DeptNo)
                                         .Average(x => x.Sal))
